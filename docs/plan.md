...
6. **Run end to end** (`migration.pipeline.run_crosswalk_pipeline`, CLI: `multinet-orbis-crosswalk`) — ties steps 1-5 together into one entrypoint that writes real artifacts to disk: `<boundary_type>_crosswalk.csv` and `<boundary_type>_validation_report.json` (geometry + crosswalk validation, plus a FIPS cross-check summary when `--fips-check` is passed). Invalid source geometry raises `InvalidSourceGeometryError` and aborts before any output is written. Unmatched records and duplicate `gers_id` claims do **not** currently block the run — see Milestone 5 below for why that's still an open decision, not a default.

24 tests passing. Fixtures use **real geometry and real GERS IDs** for Texas (region), Hays County, and Travis County, read directly from Overture's open dataset (release `2026-06-17.0`) — plus El Paso County, present on the Multinet side but deliberately omitted from the Orbis fixture, to exercise the unmatched path. The county fixture also carries each county's real Wikidata QID (Hays: `Q27018`, Travis: `Q110426`), pulled from Overture's separate `division` point-type record.
...
- Done: `validate_crosswalk`'s (and `validate_geometry`'s) reports are now real pipeline artifacts, written to disk by `migration.pipeline.run_crosswalk_pipeline` as `<boundary_type>_validation_report.json`, alongside the crosswalk itself as `<boundary_type>_crosswalk.csv`. Runnable via the `multinet-orbis-crosswalk` CLI — see `README.md`.
- Still open: decide the policy for unmatched records and duplicate `gers_id` claims: block the run (non-zero exit, no artifacts), or ship a partial crosswalk with them flagged for manual review (today's default — the pipeline always writes output and never fails on these, only on invalid source geometry). This decision should also cover whether a `fips_cross_check` disagreement factors in.
